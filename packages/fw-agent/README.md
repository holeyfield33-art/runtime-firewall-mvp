...
| `FW_AST_INCOMPLETE_POLICY` | `observe` | Only relevant when `FW_ENABLE_AST=1`. Governs what happens when a module is so densely packed with high-risk obfuscation shapes that the AST tier could not fully analyze it (an *incomplete* scan). `observe` records WARN-only telemetry and lets the module run; `quarantine` (or `block`) treats an un-analyzable suspicious module as block-tier (fail-closed). |
